...
public static class ContentEndpoints
{
    public static void MapContentEndpoints(this WebApplication app)
    {
        ...
        group.MapPost(
            ...
            async (Guid courseId, [FromForm] MaterialUploadRequest req, MaterialService service) =>
            {
                if (!Guid.TryParse(req.UploadedByUserId, out var uid))
                ...
                using (var s = File.Create(fullPath))
                    await req.File.CopyToAsync(s);

                var m = await service.UploadAsync(
                    courseId,
                    lid == Guid.Empty ? null : lid,
                    req.Title ?? req.File.FileName,
                    req.File.FileName,
                    fullPath,
        ...
        group.MapGet(
            ...
            {
                return Results.Ok(
                    await service.GetMaterialsForCourseAsync(
                        courseId,
                        lessonId,
                        fileType,
                        search,
        ...
        group.MapGet(
            ...
            async (Guid materialId, MaterialService service) =>
            {
        ...
        group.MapGet(
            ...
            {
                var m = await service.GetMaterialByIdAsync(materialId);
                if (m == null || !File.Exists(m.FilePath))
                    return Results.NotFound("File not found");
                return Results.File(
                    File.OpenRead(m.FilePath),
        ...
        group.MapPut(
            ...
            async (
                Guid materialId,
                [FromBody] UpdateMaterialRequest req,
                MaterialService service
            ) =>
            {
                var m = await service.UpdateMetadataAsync(
                    materialId,
                    req.Title,
                    req.Description,
                    req.LessonId
        ...
        // 6. REPLACE FILE
        group.MapPost(
            "/materials/{materialId}/replace",
            async (Guid materialId, IFormFile file, MaterialService service) =>
            {
        ...
        group.MapDelete(
            ...
            async (Guid materialId, MaterialService service) =>
            {
                return await service.DeleteAsync(materialId)
                    ? Results.NoContent()
                    : Results.NotFound();
            }
        );
        ...
        group.MapGet(
            "/my/materials",
            (Guid? courseId, [FromQuery] bool recentFirst) =>
            {
